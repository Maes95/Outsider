En esta sección se expondrá la mayor cantidad de información sobre el desarrollo per se\tutor{Esta expresión no aporta nada y se puede quitar 'per se'}
...
\begin{itemize}
	...
	\item RNF Despliegue y administración - La aplicación estará lista para el despliegue y accesible para poder tener información de administración del servidor
	      backend.
	\item RNF6 Documentación - Todas las capacidades de administración actuales y de futuro desarrollo serán accesible \tutor{accessibleS. Si es documentación, será que están documentadas, ¿no? ¿Dónde está esta documentación} de forma sencilla.
\end{itemize}
...
\begin{enumerate}
	\item Backend - Django. Para empezar\tutor{De nuevo, esto es muy coloquial. No empiezas a describir algo diciendo "Para empezar. Describe el servidor y sus características, esto no es una conversación}, es fundamental el uso de un servidor backend capaz de
	...
	\item Servidor websocket - Redis (Django Channels). Al hacer uso de Django y de su particular implementación de
	      websockets, se ve\tutor{Otra vez, aquí tendrías que usar 'se considera'} indispensable el uso de un servicio adicional, un servidor Redis. Este servidor es necesario para
...
ambas aplicaciones (frontend y backend), así como sus dependencias (python\tutor{Sigues poniendo Python en minúsculas y mayúsculas sin seguir ningún criterio, trata de ser consistente.} y Node.js principalmente) se ejecutan en Ubuntu a través de la interfaz
...
no tiene ninguna clase de pista adicional, se hace complicado evitar ser descubierto como Outsider. Por ende\tutor{coloquial}, la lista de palabras se hace algo más compleja, ya que, en este caso, el Outsider
debería tener otra palabra que se parezca a nivel semántico a la palabra clave, pero sin ser la propia palabra clave; \tutor{Aquí corta la frase con punto, que ya es muy larga}por ello, para generar esta lista de tuplas se ha hecho uso de ChatGPT \cite{ChatGPT} para
...
en la sección anterior de problemas, los \tutor{los qué?}\ref{subsub:resultados_ronda} pueden llegar a ser muy variados e impredecibles debido a la cantidad de combinaciones
...
Dicho esto\tutor{coloquial}, lo primer que se suele hacer en un test es crear información simulada, normalmente denominada como ``mock data''. Estos datos simulados
...
Cabría destacar que la mayoría de lógica que se testea per se\tutor{coloquial} es en su mayoría las conexiones websocket, por ello se agiliza la creación de salas
...
En el anexo (\ref{sect:testing})\tutor{Quizás te lo tenía que haber puesto antes, cuando se referencia a una Tabla, Figura y Anexo, debes utilizar el siguiente formato Anexo B.1., Figura 3.4 (en mayúsculas y sin paréntesis)} se pueden encontrar las instrucciones en detalle para preparar el entorno y ejecutar los tests.
...
probar de forma automática;\tutor{Corta aquí, frase muy larga} por ejemplo, detectar un correcto uso de colores o corroborar que existe una lógica de navegación intuitiva no son pruebas 
...
Debido a que este proyecto ha sido realizado por una sola persona, el testing manual ha sido trabajado por el propio desarrollador de la aplicación.
...
en cuenta la realización de una pequeña\tutor{no la llames pequeña, es una prueba en toda regla} prueba con usuarios externos para tener en cuenta sus opiniones.
...
En la figura (\ref{fig:res_prueba}) tal se muestra una foto del día que se realizó esta prueba. Es importante destacar que se hizo con bastantes\tutor{trata de formalizarlo un poco, ¿cuántos jugadores?} jugadores en un mismo
...
Las diferentes propuestas fueron realizadas en la aplicación final y el error solucionado. Además de esto, sirvió para observar su uso por parte de otros usuarios
...
con conexión a internet y un navegador web. El despliegue se puede hacer de muchas formas y haciendo uso de diferentes tecnologías\tutor{Corta aquí, elimina el pero} pero para 
...
proporciona capacidad informática\tutor{capacidad de cómputo} con tamaño modificable en la nube aunque en el marco del proyecto solo se hará uso de una pequeña máquina
t2.micro con Linux\tutor{matiza que es Ubuntu y su versión} como sistema operativo. Además de venir incluida en la capa gratuita de AWS, no es necesario contratar 
una máquina con más capacidad de cómputo. 

Además de la instanciación de la máquina, se propone la obtención de un dominio web para facilitar y dejar más vistoso el acceso a la aplicación
...
\tutor{A partir de aquí, creo que debería ir en la descipción de la aplicación, no en el despliegue}
...
\alumno{El resto entiendo que es hacer el resumen y las conclusiones finales, no se si ves algo más que te descuadre para retocarlo}\tutor{Salvo las notas que te he puesto, está bien}
