...
directo del término paquete o módulo 'Channels is comprised of several packages...' son prácticamente sinónimos pero como veas}\tutor{puedes dejarlo así} que se integran dentro del framework de Django.
...
virtuales\tutor{No verás en ningún sitio contenedores virtuales. Llámalos simplemente contenedores}. Estos contenedores se emplearán para hospedar los diversos servicios del sistema en servidor.\tutor{No lo hospeda,simplemente es un empaquetamiento. Si te cuesta definir Docker, trata de buscar una definición en otro TFG o más oficial, evitando inventarte cosas (lo cual, te pueden penalizar.)}

\subsection{AWS}

Amazon Web Services (AWS)~\cite{aws} es una plataforma de servicios en la nube ofrecida por Amazon. Proporciona
una amplísima gama de servicios que incluyen computación, almacenamiento, bases de datos, ... \tutor{Abusas de los ters puntos cuando en muchos casos simplemente puedes poner un etc. ya está. Como te comenté, hay que evitar usar el lenguaje natural que utilizarías en una conversación por WhatsApp por ejemplo} Estas soluciones permitirán
...
\tutor{Te ha quedado algo escueta. Aquí puedes contar las distintas iteraciones que hemos realizado apoyándote en el documento que tenemos.}
